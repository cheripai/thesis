...
def flatten_to_tiles(img, tile_size=c.IMG_SIZE):
    h, w, c = img.shape
    tiles = np.zeros(((h // tile_size) * (w // tile_size), tile_size, tile_size, c))
...
if __name__ == "__main__":
    ...
    in_tiles = np.moveaxis(flatten_to_tiles(img, c.IMG_SIZE), -1, 1)
    ...
    out_img = unflatten_tiles(out_tiles, h, w)
    imsave("output.png", out_img)
